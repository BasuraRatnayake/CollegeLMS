...
namespace CollegeLMS.Newspaper{
    public partial class showNewspapers:Form{
        public showNewspapers(){
            InitializeComponent();
            this.Height = 191;
        }
        ...
        private void ShowBooks_Click(object sender, EventArgs e, int count){//Execute Button Operation
            ...
            String[] data = new String[] {
                json[count].nmId, json[count].title, json[count].genre, json[count].published,
                json[count].lang, json[count].pic, json[count].publisher, json[count].bType
            };
            effects.showDialog(new Newspaper.showNewspaper(data));
        }
        ...
        private int getData(){//Get Data and Set to Controls
            ...
            jsonData = server.showNewspapers(txtSearch.Text, searchType);//Data from the database server
            ...
            lblRes.Text = "Found " + count[0] + " Newspapers and " + count[1] + " Magazines";

            this.Height = 214;
            if(resultCount >= 1)
                this.Height = 649;
            else
                lblRes.Text = "No Newspapers or Magazines Found";

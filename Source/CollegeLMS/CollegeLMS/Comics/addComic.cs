...
namespace CollegeLMS.Comics{
    public partial class addComic:Form{
        ...
        private Boolean getInputs(){//Get Text Based Inputs and Validate
            ...
            inputs = new String[]{
                txtBTitle.Text, cmbGenre.Text, dtpPublished.Value.ToString().Split(' ')[0], cmbLang.Text, encryption.generateCode5() + "." + ofdPicture.SafeFileName.Split('.')[1], txtbPublish.Text,cmbLan.Text
            };
        ...
        private void getImage(){//Get Image of Book from User PC
            ofdPicture.Title = "Select Picture of Comic";
        ...
        private void postInputs(){//Post Data to the Server and Database
            if(getInputs()) {
                ...
                if(server.addComic(inputs)) {//To Database server
                    fileServer.UploadFile("Comics", inputs[4], file.Length, 0, file);//Send File to Server
                    MessageBox.Show("Comic Added", "CLMS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Close();
                } else

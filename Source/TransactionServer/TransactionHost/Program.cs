...
namespace TransactionHost {
    class Program {
        protected static void login() {//Login Code
            ...
            while(username != "admin" && password != "mywins13") {
                ...
                if(username == "admin" && password == "mywins13") {
                    Console.Title = "College Library Management Transaction Server 1.0 - Not Logged In";
                    getCommands();
                } else
        ...
        protected static void getCommands() {//Establish Commands
            ServiceHost host = new ServiceHost(typeof(TransactionServer.Transactions));
            Console.Clear();
            String input = "";

            Console.WriteLine();
            Console.WriteLine();
            Console.WriteLine("\t\tWELCOME TO CLMS TRANSACTION SERVER");
            Console.WriteLine();
